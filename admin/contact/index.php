...
$getcontactQuery = "select c.* from contact c";

// tìm kiếm
if ($keyword !== "") {
    $getcontactQuery .= " where (c.name like '%$keyword%'
...
// dd($getcontactQuery);
$contact = queryExecute($getcontactQuery, true);
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0 text-dark">Quản trị contact</h1>
                        </div>
                        <!-- /.col -->
                        <div class="col-sm-6">
                            ...
                            <tbody>
                                <?php foreach ($contact as $co) : ?>
                                    <tr>
                                        ...
                                        <td><?php echo $co['message'] ?></td>
                                        <?php if ($co['status'] == -1) { ?>
                                            <td class="text-danger">Chưa trả lời</td>
                                        <?php } else if ($co['status'] == 1) { ?>
                                            <td class="text-secondary">Đã trả lời</td>
                                        <?php } ?>
                                        <td>
                                            <?php if ($_SESSION[AUTH]['role_id'] > 1) : ?>
                                                <a href="<?php echo ADMIN_URL . 'contact/content-email.php?id=' . $co['id'] ?>" class="btn btn-sm btn-success">
                                                    <i class="far fa-comment-dots"></i>
                                                </a>
                                            <?php endif; ?>
                                            <?php if ($_SESSION[AUTH]['role_id'] > 1) : ?>
                                                <a href="<?php echo ADMIN_URL . 'contact/remove.php?id=' . $co['id'] ?>" class="btn-remove btn btn-sm btn-danger">
